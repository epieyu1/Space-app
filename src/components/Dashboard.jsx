...
import { formatCurrency, convertToBase64, motivationalQuotes, PAYMENT_ACCOUNTS } from '../utils/helpers';
...
const Dashboard = ({ user, transactions, expenses, loans, bills }) => {
  ...
  const handleImg = async (name, e) => { const f = e.target.files[0]; if(f) { try { const b64 = await convertToBase64(f); await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'app_settings', 'partners'), { ...partnerImages, [name]: b64 }); } catch(e){} } };
  const copyReminder = (alert) => { const account = PAYMENT_ACCOUNTS.find(a => a.id === alert.targetAccount) || PAYMENT_ACCOUNTS[0]; const msg = `Hola ${alert.clientName}, recordatorio de pago ${alert.concept}. Cuenta: ${account.type} ${account.number} (${account.owner}).`; const ta = document.createElement("textarea"); ta.value = msg; ta.style.position="fixed"; ta.style.left="-9999px"; document.body.appendChild(ta); ta.focus(); ta.select(); try { document.execCommand('copy'); alert('Copiado'); } catch(e){} document.body.removeChild(ta); };
  const dismissAlert = (id) => { setCheckedAlerts(prev => ({...prev, [id]: true})); };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">{['Luis','Israel','Anthony'].map(p=>(<div key={p} className="bg-white p-6 rounded-3xl shadow-sm flex flex-col items-center relative group"><div className="w-20 h-20 rounded-full bg-gray-50 border-4 border-white shadow-lg mb-4 overflow-hidden flex items-center justify-center">{partnerImages[p]?<img src={partnerImages[p]} className="w-full h-full object-cover"/>:<span className="text-2xl font-bold text-gray-300">{p[0]}</span>}<label className="absolute bottom-20 right-6 bg-black text-white p-1.5 rounded-full opacity-0 group-hover:opacity-100 cursor-pointer"><Camera size={12}/><input type="file" className="hidden" onChange={e=>handleImg(p,e)}/></label></div><span className="text-xs font-bold text-[#522b85] uppercase">Socio</span><span className="font-bold text-lg">{p}</span><span className="font-black font-mono text-xl mt-2">{formatCurrency(partnerBalances[p])}</span></div>))}<div className="bg-[#522b85] p-6 rounded-3xl shadow-xl text-center text-white"><div className="w-20 h-20 bg-white/10 rounded-full flex items-center justify-center mb-4 font-black text-2xl mx-auto text-[#f7c303]">S</div><span className="text-xs font-bold uppercase opacity-60">Caja Menor (Disponible)</span><span className="font-bold text-lg block">Space</span><span className={`block font-black font-mono text-xl mt-2 ${netSpace>=0?'text-[#f7c303]':'text-red-400'}`}>{formatCurrency(netSpace)}</span></div></div>
    </div>
  );
};
