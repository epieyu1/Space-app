...
const Dashboard = ({ user, transactions, expenses, loans, bills }) => {
  ...
  const alerts = useMemo(() => {
    ...
    return bills.filter(bill => {
      ...
      return diffDays <= 3 && !checkedAlerts[bill.id];
        ...
        const [y,m,d] = bill.nextDate.split('-').map(Number);
        const billDate = new Date(y, m-1, d);
        const diffTime = billDate - new Date();
        const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
  ...
  const copyReminder = (alert) => {
    ...
    const msg = `Hola ${alert.clientName}, estamos en fecha de cobro del servicio de ${alert.concept}. Paso a recordarte y te dejo el número de cuenta: ${account.type} ${account.number} a nombre de ${account.owner}. Recuerda enviarnos el comprobante una vez realices el pago. ¡Gracias!`;
    navigator.clipboard.writeText(msg).then(() => alert('Mensaje copiado'));
  };
  ...
  const inc = transactions.reduce((a,c)=>a+Number(c.amount),0);
              ...
              <div key={alert.id} className="bg-gray-50 p-4 rounded-2xl flex flex-col md:flex-row justify-between items-center gap-4 border border-gray-100">
                <div className="flex items-center gap-4 w-full">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">{alert.concept} • <span className={alert.daysLeft < 0 ? "text-red-500 font-bold" : "text-gray-600"}>
                      {alert.daysLeft < 0 ? `Vencido hace ${Math.abs(alert.daysLeft)} días` : (alert.daysLeft === 0 ? "Vence HOY" : `Vence en ${alert.daysLeft} días`)}
                    </span></p>
                  </div>
                </div>
                <div className="flex gap-2 w-full md:w-auto">
                  <button onClick={() => copyReminder(alert)} className="flex-1 md:flex-none bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-xl font-bold text-xs flex items-center justify-center gap-2 transition-colors">
                    <MessageCircle size={16}/> Copiar WhatsApp
                  </button>
                  <button onClick={() => dismissAlert(alert.id)} className="flex-1 md:flex-none bg-gray-200 hover:bg-gray-300 text-gray-600 px-4 py-2 rounded-xl font-bold text-xs flex items-center justify-center gap-2 transition-colors">
